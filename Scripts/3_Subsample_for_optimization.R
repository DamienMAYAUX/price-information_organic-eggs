...
saveRDS(choice_situation_with_nosale_for_estimation, "Inputs/choice_situation_with_nosale_for_estimation.rds")
saveRDS(choice_situation_without_nosale_for_estimation, "Inputs/choice_situation_without_nosale_for_estimation.rds")
...
df_product_with_nosale = consumption_completed_with_nosale%>%
  select(marque, marque_simple, calibre, label, valqvol)%>%
  unique()%>%
  mutate(constant = 1)%>%
  group_by(constant)%>%
