...
df_retailer_set_period = consumption_cleaner6%>%
  count(hhid, retailer, periode)%>%
  select(-n)

saveRDS(df_retailer_set_period, "Inputs/df_hhid_retailer_set_period.rds")

df_retailer_set_year = consumption_cleaner6%>%
  count(hhid, retailer)%>%
  select(-n)%>% mutate(dummy = 1)%>%
  full_join(
    data.frame(periode = as.factor(1:13), dummy = 1)
  )%>% select(-dummy)

saveRDS(df_retailer_set_year, "Inputs/df_hhid_retailer_set_year.rds")

df_retailer_set_perfect_competition = consumption_cleaner6%>%
  select(hhid)%>% 
  unique()%>% 
  mutate(dummy = 1)%>%
  full_join(
  ...
  full_join(
    data.frame(
      retailer = consumption_cleaner6$retailer %>% unique(), 
      dummy = 1
      )
  )

saveRDS(df_retailer_set_perfect_competition, "Inputs/df_hhid_retailer_set_perfect_competition.rds")
  
df_retailer_set = df_retailer_set_perfect_competition%>%
  mutate_at(vars(periode), as.factor)
...
#### SAVING
  ...
  saveRDS(consumption_cleaner6, "Inputs/shopping_trips_with_nosale_20220115.rds")
  saveRDS(df_choice_set, "Inputs/choice_sets_with_nosale_20220115.rds")
  saveRDS(df_retailer_set, "Inputs/retailer_sets_with_nosale_20220115.rds")
  saveRDS(df_choice_situation, "Inputs/choice_situations_without_price_with_nosale_20220115.rds")

    ## SECOND VERSION, WITHOUT NOSALE

  saveRDS(consumption_cleaner6%>% filter(marque != "nosale"), "Inputs/shopping_trips_without_nosale_20220115.rds")
  saveRDS(df_choice_set%>% filter(marque != "nosale"), "Inputs/choice_sets_without_price_without_nosale_20220115.rds")
  saveRDS(df_retailer_set, "Inputs/retailer_sets_with_nosale_20220115.rds")
  saveRDS(
    df_choice_situation%>% filter(marque != "nosale", marque_chosen != "nosale"), 
    "Inputs/choice_situations_without_nosale_20220115.rds"
    )
